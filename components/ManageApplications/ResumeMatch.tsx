...
const TalentMatchProgress = ({ jobId, activeFunc }: { jobId: string,activeFunc: (idx:number)=>void }) => {
  const [currentStep, setCurrentStep] = useState("");
  const [progress, setProgress] = useState(0);
  const [error, setError] = useState("");
  const dispatch = useDispatch();
  const startMatching = async () => {  setProgress(0);  setError("");  setCurrentStep("Starting...");
    ...
    try {
      ...
      if (!reader) {  setError("Unable to read response stream."); return; }
      ...
      while (true) {
        ...
        for (const chunk of chunks) {
          try {
            ...
            // Update progress
            if (data.step && stepToProgress[data.step as keyof typeof stepToProgress] ) {
            ...
            if (data.step === "done" && data.matches) {dispatch(setApplication(data.updatedJob.applicants)); setTimeout(() => { activeFunc(1)}, 1000); }
  ...
  return (
    <div className="max-w-xl ">
        <CTABTN route={""} isFunc func={startMatching} CTA={progress==100?'Re-Match':"Start Matching"} backGround="bg-[#001E80]" width="w-[200px] max-xxsm:w-full"  height2="h-[50px] text-sm"/>
      <div className="mt-4">
